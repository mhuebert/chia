...
(defmacro peek
  ...
  [& body]
  `(binding [~'chia.triple-db.core/*access-log* nil]
     (do ~@body)))
...
#_(defmacro db-log [& body]
    `(binding [~'chia.triple-db.core/*db-log* (atom {})
               ~'chia.triple-db.core/*prevent-notify* true]
       (let [value# (do ~@body)]
         {:tx-report @~'chia.triple-db.core/*db-log*
          :value value#})))