...
(defn read-query* [cache data query-form]
  (let [__typename (:__typename data)
    ...
    #_(into {} (comp (reduce (fn [m [req-key
                                   cache-key
                                   child-keys]]
                             (let [v (get data cache-key)]
                               (assoc m req-key
                                        (if child-keys
                                          ...
                                          v)))) {})
                   (map #(parse-query-keys __typename %)))
          (cond-> query-form
                  (satisfies? g/IGraphQL query-form) (children)))
    (reduce (fn [m [req-key
                    cache-key
...
(defn read-query [cache {:as req
  ...
  (let [query-id (req-id req)
    ...
    (cond-> (read-query* cache root-data query)
            (some? query-cache)
            (merge (dissoc root-data :async/value)))))
